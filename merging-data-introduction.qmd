...
This tutorial is not meant to be exhaustive of all the different transformations and manipulations of data but is simply meant to demonstrate the power of using R when working across complex epidemiological data sets.

It is particularly ideal to perform this functions in the R coding language where they are repetitive tasks (i.e. producing regularly disease surveillance reports).
...
To begin, we import the cleaned linelist of cases from a simulated Ebola epidemic. 

If you downloaded this repository then you will see the `linelist_cleaned.rds` file in the *data/* directory ``. 
...
Those not using R can open the `linelist.csv` file in directory **data/forNonRUsers/** using excel
...
**_CAUTION:_** Typically when cleaning one should create a new column (e.g. `hosp_name_clean`), but for ease of demonstration we show modification of the old column
...
::: {.callout-caution title="Case specific"}

**_CAUTION:_** Joins are case-specific! Therefore it is useful to convert all values to lowercase or uppercase prior to joining. See the page on characters/strings.
