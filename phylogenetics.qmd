...
Much of this lesson is a shorten version of the EpiR  [Phylogenetic trees](https://www.epirhandbook.com/en/new_pages/phylogenetic_trees.html)
...
The data for this page is already saved in the directory **data/phylo**. It contains the phylogenetic tree and the metadata.
...
First, we use the `read.tree()` function from **ape** package to import a Newick phylogenetic tree file in .txt format, and store it in a list object of class "phylo". 


::: callout-tip

For anyone not comfortable with R you could view the phylogenetic tree in an online browser like https://itol.embl.de/upload.cgi. The metadata sheet can be opened in excel. These two files are in the directory **data/phylo**
...
```{r}
tree <- ape::read.tree("data/phylo/Shigella_tree.txt")
```
...
```{r}
sample_data <- import("data/phylo/sample_data_Shigella_tree.csv")
```
...
First we need to plot our tree (this can be either linear or circular - we will practice both) and store it in a new ggtree plot object `p`: We will use the sub_tree from earlier.

### Circular plot {.unnumbered}

Start with producing a circular plot of the tree
...
- [ggtree: Elegant Graphics for Phylogenetic Tree Visualization and Annotation](https://guangchuangyu.github.io/ggtree-book/short-introduction-to-r.html)
